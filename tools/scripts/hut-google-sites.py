...
def don(md: str) -> str:
    ...
    # Sites nhét chú thích nguồn dính liền vào chữ: "…hộ gia đìnhqcvn.com.vnfptshop.com.vn".
    # Tách tên miền dính đuôi câu ra thành ghi chú trong ngoặc.
    md = re.sub(r"(?<=[a-zà-ỹ])((?:[a-z0-9-]+\.)+(?:com|vn|org|net|gov|edu)(?:\.[a-z]{2})?)+",
                lambda m: f" *(nguồn: {m.group(0)})*", md)
    md = re.sub(r"\n{3,}", "\n\n", md)
    md = re.sub(r"[ \t]+\n", "\n", md)
    return md.strip()
