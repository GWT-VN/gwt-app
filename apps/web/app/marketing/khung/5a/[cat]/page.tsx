    ...
    <section className="view">
      <div className="page-head">
        <div className="eyebrow">
          <Link href="/marketing/khung/5a" style={{ color: "inherit" }}>Chiến lược 5A</Link> · Tuyến {t.cat}
        </div>
      ...
      {cases.length > 0 && (
        <>
          ...
          <div className="grid grid-2">
            {cases.map((c) => (
              <Link className="card link" href={`/marketing/kho-case/${c!.slug}`} key={c!.slug}>
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 26, flexWrap: "wrap" }}>
        {prev && (
          <Link className="btn btn-primary" href={`/marketing/khung/5a/${prev.slug}`} style={{ textDecoration: "none", background: "var(--surface)", color: "var(--ink)", border: "1px solid var(--line-2)", width: "auto" }}>
            ← {prev.cat} {prev.name}
          </Link>
        )}
        {next && (
          <Link className="btn btn-primary" href={`/marketing/khung/5a/${next.slug}`} style={{ textDecoration: "none", width: "auto" }}>
            {next.cat} {next.name} →
          </Link>
        )}
      </div>
